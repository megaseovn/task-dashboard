...
import { Plus, Edit2, Trash2, Send, Bell, LogOut, X } from 'lucide-react';
...
export default function TaskDashboard() {
  ...
  const [employees, setEmployees] = useState(() => loadFromLocalStorage('employees', [
    { id: 0, name: 'Manager', email: 'manager@gmail.com', password: '123456', role: 'manager' },
    { id: 1, name: 'Nguyễn Văn A', email: 'nguyena@gmail.com', password: '123456', role: 'employee' },
    { id: 2, name: 'Trần Thị B', email: 'tranb@gmail.com', password: '123456', role: 'employee' },
  ]));
  ...
  const handleAddEmployee = (e) => {
    e.preventDefault();
    if (!newEmployeeForm.name || !newEmployeeForm.email || !newEmployeeForm.password) {
      alert('Vui lòng điền đầy đủ thông tin!');
      return;
    }
    const newId = Math.max(...employees.map(e => e.id), 0) + 1;
    setEmployees([...employees, { ...newEmployeeForm, id: newId, role: 'employee' }]);
    setNewEmployeeForm({ name: '', email: '', password: '' });
  ...
  const handleAddTask = (e) => {
    ...
    setNewTaskForm({ title: '', type: '', project: '', status: '', deadline: '' });
  ...
  // Add demo manager if not exists
  if (!employees.find(e => e.email === 'manager@gmail.com')) {
    employees.push({ id: 0, name: 'Manager', email: 'manager@gmail.com', password: '123456', role: 'manager' });
  }
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {taskTypes.map((t, i) => (
                      <span key={i} className="inline-block bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs m-1">{t}</span>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {projects.map((p, i) => (
                      <span key={i} className="inline-block bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs m-1">{p}</span>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {workStatus.map((s, i) => (
                      <span key={i} className="inline-block bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-xs m-1">{s}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                <div className="space-y-2">
                  {employees.filter(e => e.role === 'employee').map(e => (
                    <div key={e.id} className="text-xs text-gray-600 bg-gray-50 p-2 rounded">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-bold text-gray-900">📋 Danh sách Tasks</h3>
            </div>
          ...
          {editingTaskId && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-1 block">Tiêu đề</label>
                    <input
                      type="text"
                    ...
                    {status === 'Hoàn thành' && (
                      <div className="mb-3">
                        <input
                          ...
                          value={task.link}
                          onChange={(e) => handleUpdateTask(task.id, { link: e.target.value })}
