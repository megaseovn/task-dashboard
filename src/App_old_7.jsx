...
export default function TaskDashboard() {
  ...
  const [attendanceMonth, setAttendanceMonth] = useState(new Date().toISOString().split('T')[0].slice(0, 7));
  ...
  const calculateWorkDays = (userId, monthStr) => {
    const userAttendances = attendances.filter(a => 
      a.userId === userId && a.date.startsWith(monthStr)
    );
  ...
  const calculateParkingDays = (userId, monthStr) => {
    const userAttendances = attendances.filter(a => 
      a.userId === userId && a.date.startsWith(monthStr) && isWeekday(a.date)
    );
    ...
    userAttendances.forEach(att => {
      ...
      // OT không tính tiền gửi xe
    });

    return parkingDays;
  };

  const getTaskCoverage = (userId, monthStr) => {
    const monthTasks = tasks.filter(t => 
      t.assignee === userId && t.createdAt.startsWith(monthStr)
    );
            ...
            <>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div className="flex gap-4 items-end">
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-2 block">Tháng</label>
                    <input
                      type="month"
              ...
              <div className="space-y-6">
                {(selectedAttendanceEmployee === 'all' 
                  ...
                  const workDays = calculateWorkDays(emp.id, attendanceMonth);
                  const parkingDays = calculateParkingDays(emp.id, attendanceMonth);
                  const taskCoverage = getTaskCoverage(emp.id, attendanceMonth);
                  const parkingCost = parkingDays * 50000;
                  ...
                  return (
                    <div key={emp.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                      <div className="bg-gradient-to-r from-blue-600 to-emerald-600 px-6 py-4 text-white">
                        ...
                        <p className="text-sm opacity-90">Tháng {attendanceMonth}</p>
                      </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            <div className="flex justify-between p-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded border border-amber-200">
                              ...
                              <span className="font-bold text-orange-700 text-lg">{parkingDays} × 50k = {(parkingCost / 1000).toFixed(0)}k VND</span>
                            </div>
                          </div>
                        </div>
              ...
              {selectedAttendanceEmployee === 'all' && (
                <div className="bg-gradient-to-br from-blue-600 to-emerald-600 rounded-xl shadow-lg p-6 text-white">
                  <h3 className="text-2xl font-bold mb-4 flex items-center gap-2">
                    ...
                    📊 TỔNG HỢP THÁNG {attendanceMonth}
                  </h3>
                    ...
                    <div className="bg-white/20 rounded-lg p-4 backdrop-blur">
                      ...
                      <p className="text-3xl font-bold">
                        {(employees.filter(e => e.role === 'employee').reduce((sum, emp) => 
                          sum + calculateParkingDays(emp.id, attendanceMonth) * 50000, 0) / 1000000).toFixed(1)}M VND
                      </p>
                    </div>
                    <div className="bg-white/20 rounded-lg p-4 backdrop-blur">
                      ...
                      <p className="text-3xl font-bold">
                        {employees.filter(e => e.role === 'employee').reduce((sum, emp) => 
                          sum + calculateParkingDays(emp.id, attendanceMonth), 0)} buổi
                      </p>
                    </div>
                    <div className="bg-white/20 rounded-lg p-4 backdrop-blur">
                      ...
                      <p className="text-3xl font-bold">
                        {employees.filter(e => e.role === 'employee').filter(emp => {
                          const coverage = getTaskCoverage(emp.id, attendanceMonth);
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          {todayAttendance ? (
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-emerald-600 text-sm">Loại: {
